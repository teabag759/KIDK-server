...
public class KidkScenarioTests {
        ...
        void fullScenarioTest() {
                ...
                // 확인
                assertThat(accountRepository.findById(parentAccount.getId()).get().getBalance())
                ...
                // 4. 용돈 이체 (부모 -> 자녀)
                // ==========================================
                transactionService.transfer(parentAccount.getId(), childAccount.getId(), new BigDecimal("5000"),
                                "첫 용돈");

                // 확인: 부모 95,000원, 자녀 5,000원
                assertThat(accountRepository.findById(parentAccount.getId()).get().getBalance())
                                .isEqualByComparingTo("95000");
                assertThat(accountRepository.findById(childAccount.getId()).get().getBalance())
                ...
                // ==========================================
                // 자녀: 인증 사진 제출
                MissionVerification verification = verificationService.submitVerification(mission.getId(),
                                child.getId(), "PHOTO", "clean_room.jpg");
                ...
                boolean hasRewardTx = childTransactions.stream()
                                .anyMatch(tx -> "REWARD".equals(tx.getTransactionType())
        ...
        void getMissionsForOwner() {
                // 1. 사용자 준비
                User creator = userRepository.save(User.builder().firebaseUid("p2").email("p2@test.com")
                                .userType("PARENT").name("부모2").status("ACTIVE").build());
                User owner = userRepository.save(User.builder().firebaseUid("c2").email("c2@test.com").userType("CHILD")
                                .name("자녀2").status("ACTIVE").build());
        ...
        void getMissionsCreatedBy() {
                // 1. 사용자 준비
                User creator = userRepository.save(User.builder().firebaseUid("p3").email("p3@test.com")
                                .userType("PARENT").name("부모3").status("ACTIVE").build());
                User owner = userRepository.save(User.builder().firebaseUid("c3").email("c3@test.com").userType("CHILD")
                                .name("자녀3").status("ACTIVE").build());
        ...
        void completeMission_Fail_NoAccount() {
                // 1. 유저 생성 (계좌 생성 안 함)
                User creator = userRepository.save(User.builder().firebaseUid("p4").email("p4@test.com")
                                .userType("PARENT").name("부모4").status("ACTIVE").build());
                User owner = userRepository.save(User.builder().firebaseUid("c4").email("c4@test.com").userType("CHILD")
                                .name("자녀4").status("ACTIVE").build());
                ...
                org.assertj.core.api.Assertions
                                .assertThatThrownBy(() -> missionService.completeMission(mission.getId()))
        ...
        void completeMission_Fail_AlreadyCompleted() {
                // 1. 유저 및 계좌 생성
                User creator = userRepository.save(User.builder().firebaseUid("p5").email("p5@test.com")
                                .userType("PARENT").name("부모5").status("ACTIVE").build());
                User owner = userRepository.save(User.builder().firebaseUid("c5").email("c5@test.com").userType("CHILD")
                                .name("자녀5").status("ACTIVE").build());
                ...
                // 2. 미션 생성 및 1회 완료
                MissionRequest request = MissionRequest.builder()
                                .creatorId(creator.getId()).ownerId(owner.getId()).missionType(MissionType.OTHER)
                                .title("중복테스트").rewardAmount(BigDecimal.TEN).status(MissionStatus.ACTIVE).build();
                Mission mission = missionService.createMission(request);
